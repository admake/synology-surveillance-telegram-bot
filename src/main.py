...
Улучшенная версия с корректной обработкой времени и длительности видео
"""
...
class SynologyAPI:
    ...
    def __init__(self):
        ...
        # Настройка повторных попыток для важных операций
        self.login_retry = retry(
    ...
    def get_recordings(self, camera_id: Optional[str] = None, limit: int = 10) -> List[Recording]:
                ...
                for rec in recordings_data:
                    try:
                        ...
                        recording = Recording(
                            ...
                            duration=rec.get('duration', 0) // 1000,  # Конвертируем мс в секунды
                            size=rec.get('size', 0)
                        )
    ...
    def download_full_recording(self, recording: Recording) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"📥 Начинаю скачивание записи {recording.id} "
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > 0 and downloaded % (5 * 1024 * 1024) < 8192:
                            ...
                            logger.debug(f"📥 Прогресс скачивания: {percent:.1f}% "
                                       f"({downloaded/(1024*1024):.1f} МБ / {total_size/(1024*1024):.1f} МБ)")
            
            # Проверяем размер скачанного файла
            file_size = os.path.getsize(temp_file.name)
            logger.info(f"✅ Запись {recording.id} скачана, размер: {file_size/(1024*1024):.1f} МБ")
    ...
    def get_camera_name(self, camera_id: str) -> str:
        ...
        return camera.get('name', f'Камера {camera_id}') if camera else f'Камера {camera_id}'
    ...
    def send_video(self, video_path: str, caption: str = "") -> bool:
        ...
        try:
            ...
            if file_size > self.MAX_FILE_SIZE:
                logger.warning(f"⚠️ Файл слишком большой ({file_size/(1024*1024):.1f} МБ > "
...
class StateManager:
        ...
        try:
            if self.state_file.exists():
                with open(self.state_file, 'r') as f:
                    ...
                    logger.info(f"📂 Загружено состояние: {len(self.processed_ids)} обработанных записей")
    ...
    def cleanup_old_records(self, max_age_days: int = 7) -> None:
        ...
        if len(self.processed_ids) > 1000:
            ...
            logger.debug(f"🧹 Очищены старые записи, осталось: {len(self.processed_ids)}")
        ...
        try:
            ...
            for recording in recordings:
                ...
                if recording.duration <= 0:
                    logger.warning(f"⚠️ Пропускаю запись {recording.id} без длительности")
                    continue
                
                logger.info(f"🆕 Новая запись: ID={recording.id}, "
